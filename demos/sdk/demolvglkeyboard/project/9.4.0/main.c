/**
 * @file main.c
 * @brief Main source file for the demo_lvgl_keyboard LVGL Designer application.
 *
...
void simulate_button_clicks(lv_obj_t *keyboard, const char *text) {
    ...
    for (size_t i = 0; text[i] != '\0'; i++) {
        ...
        if (c == ' ') {
            ...
            lv_obj_send_event(keyboard, LV_EVENT_CLICKED, (void *)(uintptr_t)SPACE_CHARACTER_BUTTON);
        ...
        if (c == '\n') {
            ...
            lv_timer_handler();
            // Trigger the Keyboard Click event
            lv_obj_send_event(keyboard, LV_EVENT_CLICKED, (void *)(uintptr_t)ENTER_CHARACTER_BUTTON);
            // Set Keyboard Button to be in released state
        ...
        for (int j = 0; btn_map_lower[j] != '\0'; j++) {
            ...
            if (btn_map_lower[j] == c) {
                ...
                lv_timer_handler();
                // Trigger the Keyboard Click event
                lv_obj_send_event(keyboard, LV_EVENT_CLICKED, (void *)(uintptr_t)j);
                // Set Keyboard Button to be in released state
...
int main(void)
{
    /* Do not remove this line as�� it ensures correct MCU initialization. */
    #ifdef PREINIT_SUPPORTED
    preinit();
    #endif
    ...
    lv_obj_send_event(lvgl_second_screen_ui.textarea_0, LV_EVENT_FOCUSED, NULL);
